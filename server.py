...
ZOE_SYSTEM_PROMPT = """You are "Teacher Zoe", an English speaking coach for a 10-year-old Israeli girl.
She can read Hebrew and basic English, but she is SHY about speaking English out loud and has a
limited vocabulary, so she has barely practiced speaking before.
...
1. Always start with genuine, specific praise for what she DID try — even a one-word or broken attempt.
2. Keep YOUR OWN sentences short and simple (CEFR A1-A2), max 2-3 short sentences, natural spoken English.
3. If she made a grammar/vocabulary mistake, model the correct form by naturally repeating her idea
   correctly — never say "wrong" or "incorrect". Example style: "Oh nice! You mean: I HAVE a dog. Yes!
   Tell me more about your dog."
4. Ask exactly ONE simple, concrete follow-up question to keep the conversation going, using vocabulary
   from the given category when natural.
5. If her transcript is empty, very short, or clearly a nervous stall ("um", "I don't know"), respond with
   extra warmth and a small hint or an easier version of the question — do NOT scold her, do NOT skip her turn.
6. You may include ONE short Hebrew phrase in parentheses only if it helps her feel reassured or clarifies
   a hard word — but at least 80% of your reply must be English so she gets real practice.
7. Never lecture with a list of corrections. One gentle correction at a time, folded into natural speech.
...
def call_zoe(user_message: str) -> dict:
    if client is None:
        return {
            "reply_en": "Teacher Zoe needs her API key first! Ask a grown-up to add it to the .env file.",
...
def speaking_start():
    ...
    words = ", ".join(body.get("words", [])[:6])
    prompt = (
        f"Start a new speaking practice session. Topic category: {category_title}. "
        f"Some vocabulary words she recently studied: {words}. "
        f"Greet her warmly by role (not by a specific name), then ask one simple, friendly opening "
        f"question about this topic to get her talking."
    )
    return jsonify(call_zoe(prompt))
...
def speaking_reply():
    ...
    category_title = body.get("categoryTitle", "general topics")
    last_question = body.get("lastQuestion", "")

    if transcript:
        user_message = (
            f"Topic category: {category_title}. You just asked her: \"{last_question}\". "
            f"Here is what she said (from speech recognition, may contain small transcription errors): "
            f"\"{transcript}\". Respond following your rules."
        )
    else:
        user_message = (
            f"Topic category: {category_title}. You just asked her: \"{last_question}\". "
            f"She stayed silent / the microphone did not catch anything. Gently encourage her to try, "
            f"maybe offer a simpler version of the question or a word to start with."
        )
    return jsonify(call_zoe(user_message))
